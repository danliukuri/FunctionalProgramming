...
trait Solver extends GameDef {
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): LazyList[(Block, List[Move])] = {
    def createHistory(neighbours: List[(Block, Move)], hist: List[Move]) :  LazyList[(Block, List[Move])] = {
      neighbours match {
        case Nil => LazyList()
        case h :: t => (h._1, h._2 +: hist) #:: createHistory(t, hist)
      }
    }
    createHistory(b.legalNeighbors, history)
  }
  ...
  def from(initial: LazyList[(Block, List[Move])], explored: Set[Block]): LazyList[(Block, List[Move])] = {
    initial match {
      ...
      case (b, h) #:: t =>
        val newNeighbours = newNeighborsOnly(neighborsWithHistory(b, h), explored)
