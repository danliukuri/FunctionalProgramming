...
class CodecsSuite
  ...
  test ("a 'Password' value should be encoded as a JSON object (1pt)") {
  ...
  test ("it is possible to encode and decode passwords (4pts)"){
    checkProperty(Prop.forAll(
      (secret: List[Int], public: List[Int]) => encodeAndThenDecodeProp(Password(secret, public))
    ))}
