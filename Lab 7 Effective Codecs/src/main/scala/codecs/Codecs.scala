...
trait EncoderInstances:
  ...
  given stringEncoder: Encoder[String] = Encoder.fromFunction(s => Json.Str(s))

  /** An encoder for `Boolean` values */
  // TODO Define a given instance of type `Encoder[Boolean]`
  given boolEncoder: Encoder[Boolean] = Encoder.fromFunction(b => Json.Bool(b))
...
trait DecoderInstances:
  ...
  given intDecoder: Decoder[Int] = Decoder.fromPartialFunction { case Json.Num(n) if n.isValidInt => n.toInt }

  /** A decoder for `String` values */
  // TODO Define a given instance of type `Decoder[String]`
  given stringDecoder: Decoder[String] = Decoder.fromPartialFunction { case Json.Str(s) => s }

  /** A decoder for `Boolean` values */
  // TODO Define a given instance of type `Decoder[Boolean]`
  given boolDecoder: Decoder[Boolean] = Decoder.fromPartialFunction { case Json.Bool(b) => b }
...
// Individual task
// Variant 25
case class Password(secret: List[Int], public: List[Int])

object Password extends PasswordCodecs

trait PasswordCodecs:
  /** Encoder for `Password` */
  given passwordEncoder: Encoder[Password] =
    ObjectEncoder.field[List[Int]]("secret")
      .zip(ObjectEncoder.field[List[Int]]("public"))
      .transform[Password](password => (password.secret, password.public))

  /** The corresponding decoder for `Password` */
  given passwordDecoder: Decoder[Password] =
    Decoder.field[List[Int]]("secret")
      .zip(Decoder.field[List[Int]]("public"))
      .transform[Password](j => Password(j._1, j._2))
end PasswordCodecs
...
@main def run(): Unit =
  ...
  // Password Encoding
  val password = Password(List(1,2,3), List(4,5,6))
  val passwordJson = renderJson(password)
  println(passwordJson)
  println(parseJson(passwordJson))
  // Password Decoding
  val maybeJsonPassword = parseJson(""" { "secret": [1,2,3], "public": [4,5,6] } """)
  println(maybeJsonPassword.flatMap(_.decodeAs[Password]))