...
class Empty extends PostSet {
  ...
  def mostLiked = throw new java.util.NoSuchElementException("mostLiked of EmptyPostSet")
...
class NonEmpty(elem: Post, left: PostSet, right: PostSet) extends PostSet {
   ...
   */
  def mostLiked: Post = {
    lazy val leftMost = left.mostLiked
    lazy val rightMost = right.mostLiked

    if(!left.isEmpty && leftMost.likes > elem.likes)
      if(!right.isEmpty && rightMost.likes > leftMost.likes)
        rightMost
      else
        leftMost
    else if(!right.isEmpty && rightMost.likes > elem.likes)
      rightMost
    else
      elem
  }
...
object GoogleVsApple {
  ...
  lazy val googleposts: PostSet =
    PostReader.allposts.filter(post => google.exists(word => post.text.contains(word)))

  lazy val appleposts: PostSet =
    PostReader.allposts.filter(post => apple.exists(word => post.text.contains(word)))
