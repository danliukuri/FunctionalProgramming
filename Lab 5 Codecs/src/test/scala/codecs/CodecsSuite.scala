...
class CodecsSuite
  ...
  @Test def `a 'Password' value should be encoded as a JSON object (1pt)`(): Unit = {
    val password = Password(List(1,2,3), List(4,5,6))
    val json = Json.Obj(Map(
      "secret" -> Json.Arr(List(Json.Num(1), Json.Num(2),Json.Num(3))),
      "public" -> Json.Arr(List(Json.Num(4), Json.Num(5),Json.Num(6)))
    ))

    val encoder = implicitly[Encoder[Password]]
    assert(encoder.encode(password) == json)
  }

  @Test def `it is possible to encode and decode passwords (4pts)`(): Unit =
    checkProperty(Prop.forAll((secret: List[Int], public: List[Int]) => encodeAndThenDecodeProp(Password(secret, public))))
